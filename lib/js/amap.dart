...
// rotation 类型：Number default 0	初始地图顺时针旋转的角度，取值范围 [0-360] ，默认值：0
// pitch 类型：Number default 0	初始地图俯仰角度，默认 0，最大值根据地图当前 zoom 级别不断增大， 2D 地图下无效 。
// viewMode 类型：String default'2D'	初始地图视图模式，默认为 2D ，可选 3D ，选择 3D 会显示 3D 地图效果。
// terrain 类型：Boolean default false	初始地图是否展示地形，此属性适用于 3D 地图。默认为值 false 不展示地形，可选 true ，选择 true 会展示地形效果。(注意：此属性适用于 JSAPI v2.1Beta 及以上版本)。
...
// dragEnable 类型：Boolean default true	地图是否可通过鼠标拖拽平移, 默认为 true 。此属性可被 setStatus/getStatus 方法控制
// zoomEnable 类型：Boolean default true	地图是否可缩放，默认值为 true 。此属性可被 setStatus/getStatus 方法控制
// jogEnable 类型：Boolean default true	地图是否使用缓动效果，默认值为 true 。此属性可被 setStatus/getStatus 方法控制
// pitchEnable 类型：Boolean default true	是否允许设置俯仰角度, 3D 视图下为 true , 2D 视图下无效。
// rotateEnable 类型：Boolean default true	地图是否可旋转, 图默认为 true 。
// animateEnable 类型：Boolean default true	地图平移过程中是否使用动画（如调用 panBy、panTo、setCenter、setZoomAndCenter 等函数, 将对地图产生平移操作, 是否使用动画平移的效果）, 默认为 true , 即使用动画
// keyboardEnable 类型：Boolean default true	地图是否可通过键盘控制, 默认为 true , 方向键控制地图平移，"+" 和 "-" 可以控制地图的缩放, Ctrl+“→”顺时针旋转，Ctrl+“←”逆时针旋转。此属性可被 setStatus/getStatus 方法控制
// doubleClickZoom 类型：Boolean default true	地图是否可通过双击鼠标放大地图, 默认为 true 。此属性可被 setStatus/getStatus 方法控制
// scrollWheel 类型：Boolean default true	地图是否可通过鼠标滚轮缩放浏览，默认为 true 。此属性可被 setStatus/getStatus 方法控制
// touchZoom 类型：Boolean default true	地图在移动终端上是否可通过多点触控缩放浏览地图，默认为 true 。关闭手势缩放地图，请设置为 false 。
// touchZoomCenter 类型：Boolean default1	可缺省，当 touchZoomCenter=1 的时候，手机端双指缩放的以地图中心为中心，否则默认以双指中间点为中心。
// showLabel 类型：Boolean default true	是否展示地图文字和 POI 信息。
...
// showBuildingBlock 类型：Boolean default true	是否展示地图 3D 楼块，默认 true 。
// showIndoorMap 类型：Boolean default false	是否自动展示室内地图，默认是 false 。
// skyColor 类型：(String | Array<Number>)?	天空颜色， 3D 模式下带有俯仰角时会显示
// labelRejectMask 类型：Boolean default false	文字是否拒绝掩模图层进行掩模
